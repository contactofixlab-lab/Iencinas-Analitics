...
export default function FinanzasReportesPage() {
  ...
  const handleGenerateReport = () => {
    setGenerating(true);
    // Simula generación de datos
    setTimeout(() => {
      setPreviewData([
        { concepto: 'Venta Departamento Las Condes', tipo: 'Ingreso', monto: '$320,000', fecha: '2026-06-10' },
        { concepto: 'Comisiones vendedores Q2', tipo: 'Egreso', monto: '$48,000', fecha: '2026-06-09' },
        { concepto: 'Venta Casa Lo Barnechea', tipo: 'Ingreso', monto: '$480,000', fecha: '2026-06-08' },
        { concepto: 'Marketing campaña digital', tipo: 'Egreso', monto: '$15,000', fecha: '2026-06-07' },
      ]);
      setGenerating(false);
    }, 1200);
  };
    ...
    <div className="space-y-6 page-enter">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Descarga reportes personalizados del área de finanzas</p>
        </div>
        <ProjectSelector value={proyecto} onChange={handleProyectoChange} />
      </div>

      {/* Previsualizador Grande — Arriba prominente */}
      <motion.div
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    {['Concepto', 'Tipo', 'Monto', 'Fecha'].map(h => (
                ...
                <tbody className="divide-y divide-white/10">
                  {previewData.map((row, i) => (
                    <tr key={i}>
                      <td className="px-4 py-3 text-gray-300">{row.concepto}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${row.tipo === 'Ingreso' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                          {row.tipo}
                        </span>
                      </td>
                      <td className={`px-4 py-3 font-semibold ${row.tipo === 'Ingreso' ? 'text-green-400' : 'text-red-400'}`}>{row.monto}</td>
      ...
      {/* Layout 2-Col: Reportes Prehechos (chico) + Filtros (grande) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Reportes Prehechos — Sidebar Chico (1 columna) */}
        <motion.div
        ...
        {/* Filtros + Botones — Panel Grande (2 columnas) */}
        <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        >
          ...
          <div className="flex gap-3">
            <motion.button
              ...
              style={{
                background: `linear-gradient(135deg, ${ACCENT.solid}, #15803d)`,
